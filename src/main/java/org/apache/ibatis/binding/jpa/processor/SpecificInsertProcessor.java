...
public class SpecificInsertProcessor extends ProcessorParent {
  ...
  public String process(Class<?> mapperInterface, String methodName, Configuration configuration) {
    ...
    List<Field> allFields = ClassUtil.getAllFields(parameterType).stream()
        .filter(a -> !a.isAnnotationPresent(JpaId.class) && !a.isAnnotationPresent(IgnoreField.class)).toList();
    // 获取插入字段
    ...
    if (!b) {
      ...
      tempXml = JpaXml.assembleInsetBatchSql(methodName, sqlShapValue, mapperInterface.getName(), fieldName,
          jpaTable.value(), sqlFields);
    }
