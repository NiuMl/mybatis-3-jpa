...
public class JpaXml {
  ...
  public static String assembleInsetSql(String id, String sql, String namespace) {
    return base.replace("{namespace}", namespace).replace("{method}", insert.replace("{id}", id).replace("{sql}", sql));
  }

  public static String assembleInsetBatchSql(String id, String sql, String namespace, String entities, String tableName,
      String fields) {
    return base.replace("{namespace}", namespace).replace("{method}", insertBatch.replace("{tableName}", tableName)
        .replace("{entities}", entities).replace("{fields}", fields).replace("{id}", id).replace("{sql}", sql));
  }
  ...
  public static String assembleUpdateSql(String id, String tableName, String ifCon, String namespace,
      String conditionWhere) {
    return base.replace("{namespace}", namespace).replace("{method}", update.replace("{id}", id)
        .replace("{tableName}", tableName).replace("{ifCon}", ifCon).replace("{ConditionWhere}", conditionWhere));
  }

  public static String assembleDeleteSql(String id, String tableName, String namespace, String conditionWhere) {
    return base.replace("{namespace}", namespace).replace("{method}",
        delete.replace("{id}", id).replace("{tableName}", tableName).replace("{ConditionWhere}", conditionWhere));
  }
